import { a as e, c as t, f as n, h as r, p as i } from "./editor.api2-PvII3Pyt.js";
...
function Ue(t) {
	switch (t) {
		case g.Error: return e.Error;
		case g.Warning: return e.Warning;
		case g.Information: return e.Info;
		case g.Hint: return e.Hint;
...
var Ge = class {
	...
	provideCompletionItems(e, n, i, a) {
		let o = e.uri;
		return this._worker(o).then((e) => e.doComplete(o.toString(), W(n))).then((i) => {
			if (!i) return;
			let a = e.getWordUntilPosition(n), o = new t(n.lineNumber, a.startColumn, n.lineNumber, a.endColumn), s = i.items.map((e) => {
				let t = {
					label: e.label,
				...
				return e.textEdit && (Ke(e.textEdit) ? t.range = {
					insert: K(e.textEdit.insert),
					replace: K(e.textEdit.replace)
				} : t.range = K(e.textEdit.range), t.insertText = e.textEdit.newText), e.additionalTextEdits && (t.additionalTextEdits = e.additionalTextEdits.map(q)), e.insertTextFormat === N.Snippet && (t.insertTextRules = r.CompletionItemInsertTextRule.InsertAsSnippet), t;
			});
			return {
...
function K(e) {
	if (e) return new t(e.start.line + 1, e.start.character + 1, e.end.line + 1, e.end.character + 1);
}
function Ke(e) {
	return e.insert !== void 0 && e.replace !== void 0;
}
function qe(e) {
	let t = r.CompletionItemKind;
	switch (e) {
...
function $e(e) {
	switch (e) {
		case I.Read: return r.DocumentHighlightKind.Read;
		case I.Write: return r.DocumentHighlightKind.Write;
		case I.Text: return r.DocumentHighlightKind.Text;
	}
...
function rt(e) {
	...
	let t = [];
	for (let r in e.changes) {
		let i = n.parse(r);
		for (let n of e.changes[r]) t.push({
			resource: i,
			versionId: void 0,
			textEdit: {
				range: K(n.range),
...
function ut(e) {
	switch (e) {
		case m.Comment: return r.FoldingRangeKind.Comment;
		case m.Imports: return r.FoldingRangeKind.Imports;
		case m.Region: return r.FoldingRangeKind.Region;
	}
}
var dt = class {
	constructor(e) {
