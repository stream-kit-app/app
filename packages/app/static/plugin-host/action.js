// src/lib/core/action/handler-field.ts
function createHandlerFields(definitions, stored) {
  return (definitions ?? []).map((definition) => {
    const existing = stored?.find((field) => field.key === definition.key);
    return {
      id: existing?.id ?? crypto.randomUUID(),
      key: definition.key,
      value: existing?.value ?? migrateOneOfFieldValue(definition, stored) ?? initHandlerFieldValue(definition)
    };
  });
}
function initInnerHandlerFieldValue(definition) {
  ...
  if (definition.type === "text" || definition.type === "select" || definition.type === "combobox" || definition.type === "select-file-or-folder" || definition.type === "code") {
    return "";
  }
  return false;
}
function initHandlerFieldValue(definition) {
...
function migrateOneOfFieldValue(definition, stored) {
  ...
  for (const migration of definition.migrateFrom) {
    const legacyValues = /* @__PURE__ */ new Map();
    for (const legacyKey of migration.keys) {
      ...
      legacyValues.set(variantId, legacyField.value);
    }
    ...
    for (const legacyKey of migration.keys) {
      ...
      if (variantId && typeof legacyValue === "string" && legacyValue.trim() && !isInnerHandlerFieldValueEmpty(
    ...
    const values = {};
    for (const variant of definition.variants) {
      values[variant.id] = legacyValues.get(variant.id) ?? initInnerHandlerFieldValue(variant.field);
    }
    return {
...
function isInnerHandlerFieldValueEmpty(definition, value) {
  if (!definition) {
    return true;
  }
  if (definition.type === "one-of") {
  ...
  return isHandlerFieldValueEmpty(
    { ...definition, key: "inner" },
    value
  );
}
function getHandlerFieldDefinition(definitions, key2) {
  return definitions?.find((field) => field.key === key2);
}
function isHandlerFieldValueEmpty(definition, value) {
  if (definition.type === "one-of") {
...
function collectLegacyHandlerFields(group) {
  return group.children.flatMap((child2) => {
    if (child2.kind === "condition") {
      return [child2];
    }
    return collectLegacyHandlerFields(child2);
  });
}
function migrateLegacyHandlerFields(stored) {
  if (stored.fields) {
  ...
  return collectLegacyHandlerFields(stored.config).map((leaf) => ({
    id: leaf.id,
    key: leaf.key,
    value: typeof leaf.value === "object" && leaf.value !== null && "value" in leaf.value ? String(leaf.value.value) : leaf.value
  }));
}

// src/lib/core/action/action-handler.svelte.ts
var ActionHandler = class _ActionHandler {
  id;
  definition;
  fields = $state([]);
  ...
  getField(key2) {
    return this.fields.find((field) => field.key === key2);
  }
  getFieldDefinition(key2) {
    return getHandlerFieldDefinition(this.definition.fields, key2);
  }
  getFieldError(fieldId, errors) {
  ...
  static clone(source2) {
    return new _ActionHandler(source2.definition, {
      fields: structuredClone($state.snapshot(source2.fields))
    });
  }
};

// src/lib/utils.ts
function slugify(value, fallback2 = "item") {
  const slug = (value ?? fallback2).trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  return slug || fallback2;
}
function uniqueSlug(value, used, fallback2 = "item") {
  const base = slugify(value, fallback2);
  let slug = base;
  let suffix = 2;
  while (used.has(slug)) {
    slug = `${base}-${suffix}`;
...
var HandlerDefinition = class {
  ...
  constructor(props) {
    ...
    props.children?.forEach((child2) => this.children.add(child2, { idScope: this.id }));
  }
  get isGroup() {
  ...
  setAvailable(available) {
    ...
    for (const child2 of this.children.items) {
      child2.setAvailable(available);
    }
  }
};
function resolveFieldDefinitions(fields) {
  const usedKeys = /* @__PURE__ */ new Set();
  return fields?.map((field) => {
    return {
      ...field,
      key: "key" in field && typeof field.key === "string" ? field.key : uniqueSlug(field.name, usedKeys, "field")
    };
  });
}
...
async function runHandlerChain(handlers, action2, context, callbacks) {
  const run3 = async (index2) => {
    if (index2 >= handlers.length) {
      return;
    }
    ...
    const next2 = () => {
      ...
      called = true;
      callbacks?.onHandlerComplete?.(handler, index2);
      resolveNext?.();
    };
    const nextPromise = new Promise((resolve) => {
      resolveNext = resolve;
    });
    callbacks?.onHandlerStart?.(handler, index2);
    try {
      const result = handler.definition.execute(action2, handler, context, next2);
      ...
      callbacks?.onHandlerComplete?.(handler, index2);
      callbacks?.onHandlerError?.(handler, index2, error);
...
// src/lib/i18n.ts
var [getContext2, setContext2] = createContext();
var globalI18n = null;
function translate(key2, params) {
  if (!globalI18n) {
    return key2;
  }
  return globalI18n.t(
...
// src/lib/core/action/validate-form.ts
function isFieldValueEmpty(definition, value) {
  ...
  if (definition.type === "text-select-text") {
    const compound2 = value;
    return !compound2.path.trim() || !compound2.type.trim() || !compound2.value.trim();
  }
  const compound = value;
  return !compound.type.trim() || !compound.value.trim();
}
function validateHandlerFields(fields, definitions) {
  const errors = {
    fieldErrors: {},
    missingFields: []
  };
...
function isCompoundConditionValue(definition, value) {
  return (definition.type === "select-text" || definition.type === "text-select-text") && typeof value === "object" && value !== null;
}
function initConditionValue(definition) {
  if (definition.defaultValue !== void 0) {
    return isCompoundConditionValue(definition, definition.defaultValue) ? { ...definition.defaultValue } : definition.defaultValue;
  }
  if (definition.type === "text-select-text") {
...
function getConditionDefinition(definitions, key2) {
  return definitions?.find((condition) => condition.key === key2);
}
function addConditionToGroup(group, conditionKey, definitions) {
  ...
  group.children.push({
    ...
    ...group.children.length > 0 ? { operator: "and" } : {}
  });
}
function addGroupToRoot(group) {
  if (group.id !== "root") {
    return;
  }
  group.children.push({
...
function normalizeConditionGroupOperators(group) {
  for (const [index2, child2] of group.children.entries()) {
    if (index2 === 0) {
      delete child2.operator;
    } else if (!child2.operator) {
      child2.operator = "and";
    }
    if (child2.kind === "group") {
      normalizeConditionGroupOperators(child2);
    }
  }
}
function removeConditionChild(group, index2) {
  group.children.splice(index2, 1);
  normalizeConditionGroupOperators(group);
}
function setConditionOperator(node, operator) {
