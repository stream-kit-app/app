...
import { ActionHandler } from '../action/action-handler.svelte';
import { HandlerDefinition } from '../action/handler/handler-definition.svelte';
import { migrateLegacyHandlerFields } from '../action/handler-field';
...
import { normalizeCommandNames, saveCommand, updateCommandEnabled } from '$db/repositories/commands';

import CommandForm from '$lib/components/core/commands/command-form.svelte';
...
export class Command {
	...
	async delete(): Promise<void> {
		...
		await getApp().commands.delete(this.id);
		this.close();
	}
	...
	async save(): Promise<boolean> {
		...
		if (wasNew && row) {
			...
			app.commands.items = app.commands.items.map((item) => (item.id === row.id ? this : item));
		}
