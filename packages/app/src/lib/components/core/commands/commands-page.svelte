<script lang="ts">
	...
	import { CommandCard } from '$lib/components/core/commands';
	...
	const selectableCommands = $derived(app.commands.items.filter((command) => command.id != null));
	...
	async function enableSelected(): Promise<void> {
		await app.commands.setEnabledBulk([...selectedIds], true);
		clearSelection();
	}

	async function disableSelected(): Promise<void> {
		await app.commands.setEnabledBulk([...selectedIds], false);
		clearSelection();
	}
	...
	async function deleteSelected(): Promise<void> {
		...
		await app.commands.deleteBulk([...selectedIds]);
		clearSelection();
	}
</script>
...
<Container class="px-6 py-6" size="md">
	...
	<div class="mt-8 flex flex-col gap-2">
		{#if app.commands.items.length === 0}
			<p class="text-sm text-dark-300">{t('No commands added yet.')}</p>
		{:else}
			{#each app.commands.items as command (command.id)}
				{#if command.id != null}
					<CommandCard
						{command}
