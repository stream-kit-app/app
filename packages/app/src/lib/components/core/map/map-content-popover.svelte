<script lang="ts">
	...
	const trimmedMapName = $derived(mapName.trim());
	const mapsApi = $derived(getApp().plugins.tryGet<CorePluginApi>('core')?.maps);
	const lifetime = $derived(
		trimmedMapName ? mapsApi?.getLifetime(trimmedMapName) : undefined
	);
	const canView = $derived(Boolean(trimmedMapName && lifetime));
	const entries = $derived.by(() => {
		if (!open || !canView || !mapsApi || !trimmedMapName) {
			return [];
		}

		return mapsApi.listEntries(trimmedMapName);
	});
</script>
...
<PopoverRoot bind:open>
	<PopoverTrigger>
		{#snippet child({ props }: { props: Record<string, unknown> })}
			<Button
				...
				icon="ri:map-2-line"
				disabled={!canView}
				aria-label={t('View map contents')}
	...
	<PopoverContent align="end" class="w-80 p-4">
		<div class="mb-3 flex flex-col gap-1">
			<p class="text-xs font-semibold text-dark-200">{t('Map contents')}</p>
			{#if trimmedMapName}
		...
		{#if entries.length === 0}
			<p class="py-2 text-xs text-dark-400">{t('This map has no entries yet.')}</p>
		{:else}
