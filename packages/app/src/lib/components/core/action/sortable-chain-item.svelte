<script lang="ts">
	...
	const effectiveType = $derived(handlerChainDnd ? 'handler' : sortableType);
	const effectiveAccept = $derived(handlerChainDnd ? 'handler' : sortableType);
	const hasTrailing = $derived(trailingContent != null);
