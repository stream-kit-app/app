<script lang="ts">
	...
	import { Eyebrow, Panel, SectionRule } from '@stream-kit/ui/blueprint';
...
<section>
	<div class="px-6 pt-10 pb-8">
		<a
			href="/plugins"
			class="mb-4 inline-flex cursor-pointer items-center gap-1 font-mono text-[11px] tracking-[0.14em] text-muted-foreground uppercase hover:text-foreground"
		>
			<Icon icon="ri:arrow-left-line" class="size-3.5" />
			Back to marketplace
		</a>

		<Panel crosshairs class="flex flex-col gap-5 p-5 sm:flex-row sm:items-start sm:p-6">
			<div
				class="flex size-14 shrink-0 items-center justify-center border border-rule text-primary"
				aria-hidden="true"
			>
			...
			<div class="min-w-0 flex-1">
				<div class="mb-2">
					<Eyebrow index="01">Plugin</Eyebrow>
				</div>
				<div class="flex flex-wrap items-center gap-2">
					<h1 class="font-outfit text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
						{plugin.name}
					</h1>
				...
				<div class="mt-3 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
					...
					{#if plugin.streamKitVersion}
						<span
							class="inline-flex items-center border border-rule bg-dark-900 px-2.5 py-1 font-mono text-[11px] tracking-wide text-muted-foreground"
						>
...
<section>
	<div class="px-6 py-10 pb-20">
		<Panel>
			<div class="flex gap-1 border-b border-rule px-2 sm:px-4">
				<a
					href={tabHref('overview')}
					class="cursor-pointer border-b-2 px-4 py-3 text-sm font-medium transition-colors {tab ===
					'overview'
				...
				>
					Reviews
					{#if plugin.ratingCount > 0}
						<span class="ml-1 font-mono text-xs text-muted-foreground">({plugin.ratingCount})</span>
					{/if}
				</a>
			</div>
