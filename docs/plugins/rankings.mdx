...
| **Tiers & Ranks** | Create tiers and rank milestones (global point thresholds). Click a rank to edit its name, points, icon, or see who holds it. |
...
- **Users in this rank** — a sorted list of everyone currently at that milestone (points, watch time, platform). Click a user to open their detail modal.

## Watch time

When **Watch time rewards** is enabled (plugin settings), the plugin tracks active Twitch chatters via the Helix chatters list (with IRC join/part as a supplement) and awards points on a configurable interval **only while your Twitch channel is live**. Offline chatters do not earn watch-time points or rank ups from this path. Twitch must be connected; reconnect Twitch after updating Stream Kit if watch time stayed at `0m`.

| Setting | Default | Description |
| --- | --- | --- |
| Watch time rewards | on | Enable automatic watch-time points while live |
