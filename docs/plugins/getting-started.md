...
For the full API reference, see [Plugin authoring API](./api.md). For day-to-day workflow details, see [Plugin development](./development.md).
...
Copy [`packages/plugin-template/`](../../packages/plugin-template/) to your own project, or work inside the monorepo.

Start from [`src/minimal.ts`](../../packages/plugin-template/src/minimal.ts) for a short example, or use [`src/index.ts`](../../packages/plugin-template/src/index.ts) when you need the full declarative page block showcase.
...
In the monorepo, use the shared Vite config from [`plugins/create-vite-build-config.js`](../../plugins/create-vite-build-config.js).
...
1. Start the app: `pnpm --filter @stream-kit/app tauri dev`
2. Build your plugin in watch mode: `pnpm --filter @stream-kit/plugin-template dev`
3. Link the plugin:
...
Users install the zip from the **Plugins** page. See [Plugin updates](./updates.md) for self-hosted update manifests.
...
For native file pickers, use `app.fs.select({ type: 'file' | 'folder', filters?: [...] })`.
...
- [Plugin authoring API](./api.md) — full contract, lifecycle hooks, widgets, i18n
- [Plugin development](./development.md) — monorepo workflow, definition IDs, dev linking
- [@stream-kit/plugin package](../packages/plugin.md) — what the npm package exports
- [Plugin template](../packages/plugin-template.md) — template files and scripts
