...
Leave a field empty for no cooldown on that scope. Cooldown state is kept in memory and resets when the app restarts. Commands blocked by cooldown are ignored silently (no chat message).
