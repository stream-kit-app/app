...
- **In-app editor** — Monaco with TypeScript checking and autocomplete for `app`, `context`, and trigger `data` (narrowed to triggers on the action). `app.plugins.tryGet('twitch')` (and other first-party plugin keys) is typed to that plugin's public API.
...
export default async ({ app, context }: ScriptContext) => {
  const twitch = app.plugins.tryGet('twitch');
  const data = context[0]?.data;
  const broadcasterId =
    typeof data === 'object' && data && 'broadcasterId' in data
      ? String((data as { broadcasterId: unknown }).broadcasterId)
      : twitch?.userId;

  if (twitch && broadcasterId) {
    await twitch.sendChatMessageAsBot(broadcasterId, 'Script says hi!');
  }
};
```
