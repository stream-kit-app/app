...
Read the signed-in public profile and authenticate with email/password. Profile updates stay in the app UI; plugins get public fields only (`id`, `name`, `avatarUrl`, `verified`, optional `subscription` `{ key, name, maxFileBytes, maxStorageBytes }`).
