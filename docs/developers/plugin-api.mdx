...
| `resolveLocalPath` | Local filesystem path when mirror is on; authenticated cloud URL when off |
| `getCachedPath` | Sync lookup of a mirrored path, or `null` when not cached yet |
