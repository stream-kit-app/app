...
Declarations are generated from app source via path aliases in `tsconfig.api.json`. The plugin package does not duplicate type definitions; it mirrors the app’s public contract. When the app API changes, rebuild `@stream-kit/plugin` before publishing to npm or syncing the plugin-starter template.
