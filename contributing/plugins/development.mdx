...
description: "Stream Kit loads plugins from installed manifests. The app no longer registers first-party plugins directly at boot."
---
...
**New to plugin authoring?** Start with [Plugin getting started](/docs/plugins/getting-started) and the [plugin-starter](https://github.com/stream-kit-app/plugin-starter/generate) GitHub template.
...
See [Plugin Authoring API](/docs/plugins/api) for the full public contract.
...
External authors package plugins with the [plugin-starter](https://github.com/stream-kit-app/plugin-starter) template:

```bash
pnpm package
```

Monorepo contributors can use the internal copy:
...
If two sibling definitions would produce the same slug, set an explicit `id` on one of them. See [Plugin Authoring API](/docs/plugins/api#definition-ids).

Saved actions store these IDs. Stream Kit migrates older index-based IDs automatically when actions are loaded.

## Plugin updates

Authors publish `manifest.json` and release zips on their own host (GitHub, GitLab, etc.). Add `updateManifestUrl` and `downloadUrl` to the manifest so Stream Kit can check for and install updates. See [updates.md](/docs/plugins/updates).
