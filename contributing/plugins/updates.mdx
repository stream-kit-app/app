...
description: "Stream Kit can check installed plugins for updates by fetching each plugin's remote `manifest.json`. First-party plugins use the site manifest API; third-party "
---
...
2. Build the plugin zip (`pnpm package` in a [plugin-starter](https://github.com/stream-kit-app/plugin-starter) project, or your plugin's package script).
...
Missing `stream-kit-app/plugin-*` repos are created on the fly when the publish token may create repositories in the organisation; otherwise create them up front with `pnpm create:plugin-repos`. A plugin that fails to publish no longer aborts the others — the run continues and exits non-zero with a summary of the failed repos.
