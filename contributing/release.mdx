...
**Build Release**, **Publish npm**, and **Publish Plugins** listen for `v*` tag pushes, but GitHub does not start workflows from tags pushed with the default `GITHUB_TOKEN`. The **Release** workflow therefore dispatches all three explicitly after pushing the release tag.
